...
# Script by Koenraad Van Meerbeekd and Elisa Van Cleemput, 2019
...
dataset<-read.csv("Data_CWM_FD_POTraits_Funct.csv", header=TRUE,sep=";")
...
basisSet(psem2,direction=NULL)
summary(psem2, intercepts = F,.progressBar = F)
...
write.csv(summary(psem2, intercepts = F,.progressBar = F)$coefficients, file = "SEM.POT.logDB.withSpecies.csv")
write.csv(summary(psem4, intercepts = F,.progressBar = F)$coefficients, file = "SEM.POT.teaS.withSpecies.csv")
write.csv(summary(psem6, intercepts = F,.progressBar = F)$coefficients, file = "SEM.POT.logP.withSpecies.csv")
